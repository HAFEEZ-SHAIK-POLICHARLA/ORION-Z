...
export const getRealtimeStatus = async (): Promise<import("./types").RealtimeMetrics> => {
  const response = await fetch("/api/realtime/status");
  return json<import("./types").RealtimeMetrics>(response);
};

export const startRealtime = async (): Promise<import("./types").RealtimeMetrics> => {
  const response = await fetch("/api/realtime/start", { method: "POST" });
  return json<import("./types").RealtimeMetrics>(response);
};

export const stopRealtime = async (): Promise<import("./types").RealtimeMetrics> => {
  const response = await fetch("/api/realtime/stop", { method: "POST" });
  return json<import("./types").RealtimeMetrics>(response);
};

export const getRealtimeReadiness = async (): Promise<import("./types").ReadinessChecks> => {
  const response = await fetch("/api/realtime/readiness");
  return json<import("./types").ReadinessChecks>(response);
};
